...
: guiManager(guiProgram),
  currentPlayer(), 
  sceneManager(shaderProgram),
  audioManager(), 
  optionsManager(guiManager, *this),
  keyMap(inKeyMap) 
{
	sceneManager.addObjectToQueue(&currentPlayer);
}
...
void GameManager::startGame(const char* noteJsonPath, const char* noteSongPath)
{
	...
	for (Json::ArrayIndex i = 0; i < x; i++) {
		...
		DrawObject* noteObject = new NoteTarget(0.f, height, time, 40.f, &audioManager, &currentPlayer);
		DrawObject* noteHighlight = new NoteHighlight(time, noteObject, &audioManager);
